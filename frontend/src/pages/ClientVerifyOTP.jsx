...
const ClientVerifyOTP = () => {
  ...
  const { correo, nombre, numero } = location.state || {};
  ...
  const handleVerify = async () => {
    ...
    if (userId) {
      ...
      if (profileError) {
        setLoading(false);
        setErrorMsg('Cuenta verificada, pero hubo un error guardando el perfil. Contacta soporte.');
        return;
      }
    }
      ...
      >
        ...
        >
          {loading ? (
            ...
            <>Confirmar registro <ArrowRight size={18} /></>
          )}
        </motion.button>
