...
const ClientDietManagement = () => {
  ...
  const fetchDiets = async () => {
    ...
    try {
      ...
      // 2. Fallback por consulta directa si falla o está vacío
  ...
  const handleDeleteDiet = async (e, dietId) => {
    e.stopPropagation();
    if (!window.confirm('¿Estás seguro de que deseas eliminar esta dieta?')) return;
  ...
  const filteredAndSortedDiets = diets
    .filter(d => {
      ...
      if (activeTab === 'mine') return matchesSearch && d.creator_id === currentUserId;
      if (activeTab === 'admins') return matchesSearch && d.creator_role === 'admin';
      ...
      <main style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            ...
            <div>
              <h1 style={{ color: 'var(--text-main)', fontSize: '2rem', fontWeight: '800' }}>Dietas</h1>
              <p style={{ color: 'var(--text-muted)' }}>Explora y comparte planes nutricionales.</p>
            </div>
          </div>

          <button 
            onClick={() => navigate('/subir-dieta')} 
            style={{
        ...
        >
          ...
          <div style={{ marginBottom: '2rem' }}>
            <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem', borderBottom: '1px solid var(--bg-app)', paddingBottom: '1rem' }}>
              {['all', 'admins', 'mine'].map((tab) => (
                ...
                >
                  {tab === 'all' ? 'Todas' : tab === 'admins' ? 'Especialistas' : 'Mis dietas'}
                </button>
              ))}
            </div>
            ...
            <div style={{
              ...
              <div style={{ position: 'relative', flex: 1, minWidth: '300px' }}>
                ...
                <input 
                  type="text"
                  placeholder="Buscar dietas o autores..."
                  value={searchTerm}
          ...
          {loading ? (
            <div style={{ padding: '4rem', display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column', gap: '1rem' }}>
              ...
              <p style={{ color: 'var(--text-muted)' }}>Cargando planes nutricionales...</p>
            </div>
              ...
              <AnimatePresence>
                {filteredAndSortedDiets.map((diet, index) => (
                  <motion.div
                    ...
                    onClick={() => navigate(`/ver-dieta/${diet.id}`)}
                    style={{
                      background: 'var(--bg-app)',
                  ...
                  >
                    {/* Badge Verificado (Solo para Admins) */}
                    {diet.is_verified && (
                      <div style={{
                        position: 'absolute',
                        top: '1rem',
...
export default ClientDietManagement;
